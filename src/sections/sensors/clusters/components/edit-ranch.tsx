...
export const EditRanch = ({
    ...
    const assignToUserMutation = useMutation({
        ...
        mutationFn: ({ ranchId, ids }: { ranchId: number; ids: number[] }) =>
            assignRanchToUser(ranchId, ids),
        onSuccess: () => {
            setSelectedUsers([]);
    ...
    const unassignToUserMutation = useMutation({
        mutationKey: ['assignToUser', ranch.id],
        mutationFn: ({ ranchId, ids }: { ranchId: number; ids: number[] }) =>
            unassignRanchToUser(ranchId, ids),
        onSuccess: () => {
            setSelectedUsers([]);
            setRemoveFromRanchUsersIds([]);
            refetchUsersInRanch();
    ...
    const handleRemoveUsersChange = (event: SelectChangeEvent<string[]>) => {
        ...
        setRemoveFromRanchUsersIds(typeof value === 'string' ? value.split(',') : value);
    };

    return (
        <Drawer open={isOpen} onClose={onClose} anchor="right" sx={{ height: '100vh' }}>
            <Scrollbar>
                <Box
                    sx={{
                        width: {
                ...
                >
                    ...
                    {assignToUserMutation.isError && (
                        <Box sx={{ my: 2 }}>
                            <ErrorAlert message={`${assignToUserMutation.error}`} />
                        </Box>
                    )}
                    {unassignToUserMutation.isError && (
                        <Box sx={{ my: 2 }}>
                            <ErrorAlert message={`${assignToUserMutation.error}`} />
                        </Box>
                    )}
                    ...
                    {mutation.isSuccess && (
                        <Box sx={{ my: 2 }}>
                            <SuccessAlert message="Rancho actualizado" />
                        </Box>
                    )}
                    {assignToUserMutation.isSuccess && (
                        <Box sx={{ my: 2 }}>
                            <SuccessAlert message="Usuarios asignados" />
                        </Box>
                    )}
                    {unassignToUserMutation.isSuccess && (
                        <Box sx={{ my: 2 }}>
                            <SuccessAlert message="Usuarios removidos" />
                        </Box>
                    )}
